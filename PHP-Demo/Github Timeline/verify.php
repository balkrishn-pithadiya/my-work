<?php
    ...
    // Get email and hash from URL which is sent with mail
    if(isset($_GET['email']) && !empty($_GET['email']) && isset($_GET['hash']) && !empty($_GET['hash'])){

        // Call database and store connection string in db_conn
        ...
        $query_select = 'SELECT * FROM users_timeline WHERE email = ? AND hash = ?';
        $stmt = $db_conn->prepare($query_select);
        $stmt->bind_param('ss', $email, $hash_key);
        $stmt->execute();
        $result = $stmt->get_result();

        // If mail found then set is_verified true
        if($result->num_rows > 0){
            $query_update = 'UPDATE users_timeline SET is_verified = 1 WHERE email = ? AND hash = ?';
            ...
            echo '
                <script>alert("Email Not Found, Please Subscribe!");
                window.location.href = "index.php";
                </script>
                ';
        }
