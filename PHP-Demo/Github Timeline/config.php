...
class Dbconnection
{
    ...
    // (eg. private $db_name = 'Provide your database name')
    // (eg. private $user_name = 'db_demo')
    private $db_name = '';
    ...
    {
        // db_conn variable intialized with the connection of database
        $this->db_conn = new mysqli($this->host, $this->user_name, $this->pass, $this->db_name);
        if ($this-> db_conn -> connect_errno) {
            echo 'Connection Failed With Database: ' . $this -> db_conn -> connect_error;
            exit();
        }
    }
